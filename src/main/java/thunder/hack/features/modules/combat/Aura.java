...
public class Aura extends Module {
    ...
    private void calcRotations(boolean ready) {
        ...
        if (rotationMode.is(Mode.LonyGrief)) {
            Vec3d lonyVec = mc.player.isFallFlying() ? target.getEyePos() : getLegitLook(target);
            computeLonyGriefRotation(lonyVec);
            return;
        }
    ...
    public enum Mode {
        Interact, Track, Grim, None, LonyGrief
    }
    ...
    private void computeLonyGriefRotation(Vec3d targetVec) {
        if (targetVec == null) return;
        float delta_yaw = wrapDegrees((float) wrapDegrees(
        ...
        float delta_pitch = ((float) (-Math.toDegrees(Math.atan2(
            ...
            Math.sqrt(Math.pow(targetVec.x - mc.player.getX(), 2) +
                      Math.pow(targetVec.z - mc.player.getZ(), 2))
        )))) - rotationPitch;
        float rotationDifference = (float) Math.hypot(Math.abs(delta_yaw), Math.abs(delta_pitch));
        if (rotationDifference == 0) return;
        long timeSinceAttack = System.currentTimeMillis() - lastAttackTime;
        double timeForYaw = timeSinceAttack / 60.0;
        double timeForPitch = timeSinceAttack / 40.0;
        float attackJitterMult = (timeSinceAttack < 250) ? 0.3f : 1.0f;
        float jitterYaw = (float)(lonyRandomLerp(3f, 6f) * Math.cos(timeForYaw)) * attackJitterMult;
        float jitterPitch = (float)(lonyRandomLerp(2f, 4f) * Math.sin(timeForPitch)) * attackJitterMult;
        boolean visible = Managers.PLAYER.checkRtx(rotationYaw, rotationPitch,
            getRange() + aimRange.getValue(), getRange() + aimRange.getValue(), rayTrace.getValue());
        float lerp;
        if (!visible) { lerp = lonyRandomLerp(0.5f, 0.7f); }
        else { lerp = lonyRandomLerp(0.25f, 0.35f); }
        if (timeSinceAttack < 250) {
            lerp = lonyRandom.nextFloat() < 0.1f ? lonyRandomLerp(0.5f, 0.8f) : lonyRandomLerp(0.3f, 0.45f);
        }
        float lineYaw = Math.abs(delta_yaw / rotationDifference) * 180f;
        float linePitch = Math.abs(delta_pitch / rotationDifference) * 180f;
        float moveYaw = MathHelper.clamp(delta_yaw, -lineYaw, lineYaw);
        float movePitch = MathHelper.clamp(delta_pitch, -linePitch, linePitch);
        float newYaw = MathHelper.lerp(lerp, rotationYaw, rotationYaw + moveYaw) + jitterYaw;
        float newPitch = MathHelper.clamp(
            MathHelper.lerp(lerp, rotationPitch, rotationPitch + movePitch) + jitterPitch, -90f, 90f);
        double gcdFix = Math.pow(mc.options.getMouseSensitivity().getValue() * 0.6 + 0.2, 3.0) * 1.2;
        rotationYaw = (float)(newYaw - (newYaw - rotationYaw) % gcdFix);
        rotationPitch = (float)(newPitch - (newPitch - rotationPitch) % gcdFix);
        ModuleManager.rotations.fixRotation = rotationYaw;
        lookingAtHitbox = Managers.PLAYER.checkRtx(rotationYaw, rotationPitch,
            getRange(), getWallRange(), rayTrace.getValue());
    }

    private float lonyRandomLerp(float min, float max) {
        return MathHelper.lerp(lonyRandom.nextFloat(), min, max);
    }
